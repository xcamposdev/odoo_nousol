...
class ProductTemplate(models.Model):
    ...
    def prestashop_to_odoo_import_product(self, warehouse_id=False, prestashop_store_id=False):
        ...
        try:
                    ...
                    if prod_id:
                        ...
                        if response_status:
                            ...
                            for product_dict in product_dicts:
                                ...
                                if not product_id:
                                    product_detail_id = self.env['prestashop.product.detail'].search(
                                        [('prestashop_product_id', '=', product_dict.get('id'))], limit=1)
                                    product_id = product_detail_id and product_detail_id.product_id

                                template_title = product_dict.get('name')
                                category_ids = product_dict.get('associations') and product_dict.get(
                                    'associations').get('categories')
                                attribute_ids = product_dict.get('associations') and product_dict.get(
                                    'associations').get('product_option_values')
                                stock_availables = product_dict.get('associations') and product_dict.get(
                                ...
                                prestashop_store_id.create_prestashop_operation_detail('product', 'import',
                                                                                            ...
                                                                                            process_message)
                                self._cr.commit()
                                if product_tmpl_id and attribute_ids and product_id.filtered(
                                        lambda pp: pp.with_user(1).product_variant_count <= 1):
                                ...
                                if not self.env['prestashop.product.detail'].search(
                                        [('prestashop_product_id', '=', product_dict.get('id')),('prestashop_store_id','=',prestashop_store_id.id)]):
                            ...
                            process_message = "Getting an Error In Import Product Response".format(
                                product_response_data)
                            prestashop_store_id.create_prestashop_operation_detail('product', 'import', req_data,
                                                                                        product_response_data,
                                                                                        product_operation_id,
            ...
            prestashop_store_id.create_prestashop_operation_detail('product', 'import', response_data,

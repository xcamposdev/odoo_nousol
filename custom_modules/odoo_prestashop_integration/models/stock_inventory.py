...
class PrestashopStockInventory(models.Model):
    ...
    def prestashop_to_odoo_import_stock_inventory(self, warehouse_id=False, prestashop_store_id=False):
        ...
        product_operation_id = prestashop_store_id.create_prestashop_operation('stock', 'import', prestashop_store_id, 'Processing...')
        try:
                ...
                if response_status:
                    ...
                    for stock_inventory in product_stock_inventory:
                        ...
                        if product_detail_id and product_detail_id.product_id.id:
                            ...
                            continue
                        # data_find = list(_data for _data in inventory_lines if _data[2]['product_id'] == product_detail_id.product_id.id)
                        # if data_find:
                        #     test = ""
                        ...
                        prestashop_store_id.create_prestashop_operation_detail('stock', 'import', api_operation, response_data, product_operation_id, False, product_stock_inventory)
                else:
                    process_message = "Import Product Stock Failed %s"%(product_detail_id and product_detail_id.product_id.name)
                    prestashop_store_id.create_prestashop_operation_detail('stock', 'import', api_operation, response_data, product_operation_id, True, process_message)

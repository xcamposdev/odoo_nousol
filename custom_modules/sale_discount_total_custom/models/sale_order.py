...
class SaleOrder(models.Model):
    ...
    def _compute_general_discount_old(self):
        for so in self:
            i = 0
    ...
    def _compute_general_discount(self):
        for so in self:
            ...
            so.amount_total = amount_tax + amount_untaxed
            if(so.general_discount != 0.0):
                so.amount_total = so.amount_total - (so.amount_total * (so.general_discount/100.0))
